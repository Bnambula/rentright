...
/* ──── PROPERTY CARD ──── */
function PropCard({l,onView,onBook,onAuth}){
  const {user}=useAuth()
  ...
  return(
    <div className="prop-card" onClick={()=>onView(l)}>
      ...
      <div className="prop-card-body">
        ...
        <button className="btn btn-forest btn-sm btn-full" style={{marginTop:10}} onClick={e=>{e.stopPropagation();if(!user)onAuth('client');else onBook(l)}}>
  ...
  return(
    <Modal open={open} onClose={onClose} size="modal-lg">
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
        <button className="btn btn-forest btn-full" onClick={()=>user?onBook(l):onAuth('client')}>📅 Book viewing</button>
        <button className="btn btn-gold btn-full" onClick={()=>user?onBook(l):onAuth('client')}>🔓 Unlock contact</button>
      </div>
    </Modal>
  )
}
  ...
  return(
    <Modal open={open} onClose={()=>{setView('detail');onClose()}} size="modal-lg" title={j.cat+' Job'} sub="Review scope, then accept or counter offer">
      ...
      {view==='counter'&&<>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
          <button className="btn btn-outline" onClick={()=>setView('detail')}>← Back</button>
          <button className="btn btn-gold" disabled={!counterVal} onClick={()=>{const amt=parseInt(counterVal)||0;if(amt%1000!==0)return showToast('Counter offer must be divisible by 1,000 (e.g. 45,000)','error');if(amt>999999)return showToast('Max 6 digits (UGX 999,999)','error');showToast(`Counter offer of ${fmt(amt)} sent`);setView('detail');onClose()}}>Send counter offer</button>
        </div>
      </>}
      {view==='ask'&&<>
    ...
    <>
      ...
      <section className="section" id="sec-properties" style={{background:'var(--white)'}}>
        <div className="container">
          ...
          <div className="grid-auto">{filtered.map(l=><PropCard key={l.id} l={l} onView={setViewL} onBook={setBookL} onAuth={openAuth}/>)}</div>}
        </div>
      </section>
